        ...
        <main>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel">Input Data Muzakki</h5>
                            <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="/dashboard/muzaki" method="POST">
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3"><label for="exampleFormControlInput1">Nama Muzakki</label><input class="form-control" name="nama" id="nama" type="text" placeholder="Nama Muzakki" autofocus required></div>
                                <div class="mb-3"><label for="exampleFormControlInput1">Jumlah Tanggungan</label><input class="form-control" name="jumlah_tanggungan" id="tanggungan" type="number" required></div>
                                <div class="mb-3"><label for="exampleFormControlInput1">Keterangan</label><textarea class="form-control" name="keterangan" id="keterangan"  placeholder="Boleh Dikosongkan"> </textarea> </div>
                            </div>
            ...
            <div class="card-body">
                <table id="datatablesSimple">
                    ...
                    @foreach($muzakki as $mz )


                    <tr>
                        <td>{{$loop->iteration}}</td>
                        <td>{{$mz->nama}}</td>
                        <td>{{$mz->jumlah_tanggungan}}</td>
                        <td>{{$mz->keterangan}}</td>
                        <td>
                            <div class="col text-center ">
                                <form action="/dashboard/muzaki/{{$mz->id}}" method="post" >
                                    @csrf
                                    @method('delete')
                        ...
                        <td>
                            <div class="col text-center">
                                <a  href="/dashboard/muzaki/{{$mz->id }}" class="btn btn-warning">Edit</a>
                            </div>
                        </td>
                        <td>
                            <div class="col text-center">
                                <a  href="/dashboard/muzaki/bayar/{{$mz->id}}" class="btn btn-info">Bayar</a>
                            </div>
                        </td>
                    </tr>
